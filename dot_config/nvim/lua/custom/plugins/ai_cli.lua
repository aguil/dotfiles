return {
  {
    ...
    config = function(_, opts)
      ...
      local function notify(message, level) vim.notify('[ai-cli] ' .. message, level or vim.log.levels.INFO) end

      local function get_command_binary(command) return command:match '^%s*([^%s]+)' or command end
      ...
      local function ensure_open()
        ...
        if not ai:is_open() then ai:open() end
      ...
      local function run_system(args, cwd)
        if vim.system then
          ...
          if result.code == 0 then return result.stdout or '' end

          local stderr = result.stderr and result.stderr:gsub('%s+$', '') or ''
          if stderr == '' then stderr = 'exit code ' .. tostring(result.code) end
          return nil, stderr
        end

        local cmd = shell_join(args)
        if cwd and cwd ~= '' then cmd = 'cd ' .. vim.fn.shellescape(cwd) .. ' && ' .. cmd end

        local output = vim.fn.system(cmd)
        if vim.v.shell_error == 0 then return output end
      ...
      local function format_diagnostics()
        ...
        if #diags == 0 then return 'none' end
        ...
        if #diags > max_items then table.insert(out, string.format('- ... %d more diagnostics', #diags - max_items)) end
      ...
      local function build_file_prompt(extra_prompt)
        ...
        if not file then return nil, 'no file is active in this buffer' end
      ...
      local function build_diff_prompt(staged, extra_prompt)
        ...
        if not root then return nil, 'current file is not inside a git repository' end

        local rel_file = rel_to_root(file, root)
        local args = { 'git', '-C', root, 'diff', '--no-ext-diff' }
        if staged then table.insert(args, '--staged') end
        table.insert(args, '--')
        table.insert(args, rel_file)

        local diff_output, diff_error = run_system(args)
        if not diff_output then return nil, 'unable to load git diff: ' .. diff_error end

        if diff_output:gsub('%s+', '') == '' then
          if staged then return nil, 'no staged changes for current file' end
      ...
      local function show_status()
        ...
        if not is_available then notify('install or add to PATH: ' .. binary, vim.log.levels.WARN) end
      end

      vim.keymap.set('n', '<leader>at', function() ai:toggle() end, { desc = 'AI: toggle terminal' })

      vim.keymap.set('n', '<leader>al', function() send_text(vim.api.nvim_get_current_line()) end, { desc = 'AI: send current line' })

      vim.keymap.set('v', '<leader>as', function() send_text(get_visual_selection()) end, { desc = 'AI: send selection' })

      vim.keymap.set('n', '<leader>ah', function() send_or_warn(build_here_prompt, '', nil) end, { desc = 'AI: send file+diag context' })

      vim.keymap.set('v', '<leader>ah', function() send_or_warn(build_here_prompt, '', get_visual_selection()) end, { desc = 'AI: send context + selection' })

      vim.keymap.set('n', '<leader>af', function() send_or_warn(build_file_prompt, '') end, { desc = 'AI: send current file' })

      vim.keymap.set('n', '<leader>ad', function() send_or_warn(build_diff_prompt, false, '') end, { desc = 'AI: send git diff (file)' })

      vim.keymap.set('n', '<leader>aD', function() send_or_warn(build_diff_prompt, true, '') end, { desc = 'AI: send staged diff (file)' })

      vim.keymap.set('n', '<leader>ax', function() send_text(build_diag_prompt '') end, { desc = 'AI: send diagnostics' })

      vim.keymap.set('n', '<leader>ai', function() show_status() end, { desc = 'AI: status' })

      vim.api.nvim_create_user_command('AiSend', function(command_opts) send_text(command_opts.args) end, { nargs = '+' })

      vim.api.nvim_create_user_command('AiHere', function(command_opts) send_or_warn(build_here_prompt, command_opts.args, nil) end, { nargs = '*' })

      vim.api.nvim_create_user_command('AiFile', function(command_opts) send_or_warn(build_file_prompt, command_opts.args) end, { nargs = '*' })

      vim.api.nvim_create_user_command('AiDiag', function(command_opts) send_text(build_diag_prompt(command_opts.args)) end, { nargs = '*' })

      vim.api.nvim_create_user_command('AiDiff', function(command_opts) send_or_warn(build_diff_prompt, false, command_opts.args) end, { nargs = '*' })

      vim.api.nvim_create_user_command('AiDiffStaged', function(command_opts) send_or_warn(build_diff_prompt, true, command_opts.args) end, { nargs = '*' })

      vim.api.nvim_create_user_command('AiStatus', function() show_status() end, { nargs = 0 })
    end,
  },
}
