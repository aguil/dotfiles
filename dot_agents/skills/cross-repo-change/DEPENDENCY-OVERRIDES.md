...
When a producer PR and a consumer PR are both in flight, the consumer has to
point at the producer's in-progress code. Use the **lowest rung that works**,
and always climb up before the consumer PR leaves draft.
...
1.  **Local path override** — fastest inner loop, never pushed.
    ...
    - Good for: iterating while both checkouts are editable in the same task.
...
2.  **Git branch override** — shared across machines/CI.
    ...
    - Codegen spec files (e.g. OpenAPI manifests, proto registries): pin to the
      same branch on the producer spec source.
...
Organisation-specific rules on which ref counts as "release-quality" (e.g. must
be a release tag rather than a raw SHA) belong in the work overlay
(`~/.agents/AGENTS.work.md`).

## Order of operations

Producer merges first **or** producer and consumer go green together with a
branch override, then the override is bumped to the release in a final commit on
the consumer PR. Coordinate the switch:
...
- `pubspec.lock` / `package-lock.json` / `yarn.lock` diffs that don't match the
  manifest delta
