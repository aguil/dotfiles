...
name: ci-triage
description: >-
  Structured approach to triaging a failing GitHub Actions check on a PR: find
  the run, extract the real error from logs, reproduce locally, fix in an
  isolated commit, push, and poll. Use whenever CI is red on any PR in the
  current task.
---
...
Don't fan out fixes across multiple commits — one failure, one focused commit.
...
test-results system reached via an MCP server or CLI), use those for richer log
access. Any such work-specific integration is documented in
...
If it only repros in CI, the delta is the environment: network access, generated
code freshness, or OS differences.
...
If the fix changes production code _and_ regenerated clients, those are two
commits (see `dart-cross-repo` on generated code hygiene).
...
- **Flaky test**: rerun once (`gh run rerun <run-id>`). If it flakes again, file
  a ticket and, if policy allows, mark the test as a known flake. Don't paper
  over with retries in production code.
...
- Amending the same commit after each failed attempt so history is a lie about
  the debugging process. Leave the trail; squash at merge time if the project's
  policy does that.
