...
| Git behavior               | diff/merge defaults (`kdiff3`/diffview mergetool via Neovim), delta pager, core options, global ignores | credential helper and keychain integration                   | `user.name`, `user.email`, signing key                        | work/personal identity split is required                                                                      |
...
| Terminal emulator settings | baseline terminal behavior where portable                                                               | Windows Terminal profile, iTerm/mac terminal choices         | none                                                          | Oh My Posh icons need a **Nerd Font** on the host that paints the window; see **`docs/prompt-font-setup.md`** |
